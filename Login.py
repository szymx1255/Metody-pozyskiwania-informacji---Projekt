...
def log_exception(logger: logging.Logger, exc: Exception, context: str | None = None) -> None:
	...
	Parametry:
	  - logger: obiekt logging.Logger
	  - exc: wyjątek który chcemy zapisać
	  - context: opcjonalny krótki opis kontekstu (np. nazwa funkcji, user id)
	"""
...
def log_exceptions(fn):
	...
	def wrapper(*args, **kwargs):
		...
		try:
			...
			# Krótki kontekst: nazwa funkcji i argumenty (nie loguj wrażliwych danych)
			ctx = f"{fn.__name__} args={args} kwargs={{{', '.join(k+':...' for k in kwargs)}}}"
			log_exception(logger, e, context=ctx)
			# Reraise żeby zachować zachowanie, można też zwrócić None jeśli preferujesz
			raise
	return wrapper
...
def timed(logger: logging.Logger, name: str):
	...
	Przykład:
	  with timed(logger, "login_attempt"):
		  do_something()
	"""
	class _Timer:
		def __enter__(self):
...
if __name__ == "__main__":
	...
	def simulate_login(user: str):
		"""Prosty przykład funkcji, która może zgłaszać wyjątek.

		W prawdziwej aplikacji tutaj byłaby logika auth/DB itd.
		"""
		if not user or user == "bad":
			raise ValueError("Nieprawidłowe dane logowania")
		return {"user": user, "status": "ok"}

	# Demonstracja: mierzymy czas i logujemy wyjątek
	try:

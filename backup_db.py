...
def backup_db(src_path: str | Path, backups_dir: str | Path = "data/backups", keep: int = 7) -> str:
    """
    Kopiuje plik bazy src_path do katalogu backups_dir z timestampem.
    Zwraca ścieżkę do backupu. Usuwa starsze kopie pozostawiając 'keep' najnowszych.
    """
    src = Path(src_path)
    if not src.exists():
...
def restore_db(backup_file: str | Path, target_db: str | Path) -> None:
    """
    Przywraca backup_file do target_db, uruchamia PRAGMA integrity_check i
    podnosi RuntimeError jeśli check != 'ok'.
    """
...
if __name__ == "__main__":
    ...
    if args.action == "backup":
        ...
        # przy restore: dst argument traktujemy jako target_db path

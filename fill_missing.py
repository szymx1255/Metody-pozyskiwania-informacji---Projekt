...
COLUMNS = ["temperature", "wind_speed", "weather_code"]  # kolumny do imputacji
...
def fill_missing(db_path: Path, do_backup: bool = True):
    ...
    cur = conn.cursor()
    # pobierz wiersze z brakami, uporządkowane po location/timestamp
    ...
    for r in rows:
        ...
        updates = {}
        for col in COLUMNS:
            # pobierz aktualną wartość (może być None)
            cur_val = None
            if col == "temperature":
                cur_val = temp
            elif col == "wind_speed":
